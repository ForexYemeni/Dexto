      ...
      <div className="relative">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className={`text-[10px] px-2 py-1 rounded-full flex items-center gap-1 ${
            isMiningStarted ? 'bg-green-500/20 text-green-400' : 'bg-amber-500/20 text-amber-400'
          }`}>
        ...
        {/* Countdown - show when mining is active */}
        {isMiningStarted ? (
          <div className="text-center mb-4">
            <p className="text-[10px] text-white/40 mb-1">
              {locale === 'ar' ? 'الوقت المتبقي للتعدين' : 'Time Until Next Cycle'}
            </p>
            ...
            <p className="text-[10px] text-white/30 mt-1">
              {locale === 'ar'
                ? `الدورة تنتهي في الموعد المحدد من الإدارة`
                : `Cycle ends at admin-set time`}
            </p>
          </div>
        ) : (
